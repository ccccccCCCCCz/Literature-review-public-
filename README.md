...
# 矿场自动驾驶卡车
## 1.Scenarios Engineering driven Autonomous Transportation in Open-Pit Mines(2023)--->http://arxiv.org/abs/2405.00690
本研究提出了一种针对露天矿山的自主矿用卡车场景工程（SE）方法。SE 从四个关键组件提高了自动驾驶卡车的可信度和稳健性：场景特征提取器、智能和索引 （I&I）、校准和认证 （C&C） 以及验证和确认 （V&V）。场景特征提取器是一种全面的管道方法，可捕获复杂挖掘场景中的复杂交互和潜在依赖关系。I&I有效提升了训练数据集的质量，为矿区自主交通奠定了坚实基础。C&C 以自动驾驶交通中使用的智能系统的内在监管、功能和贡献为基础，以与现实世界中的交通参与者保持一致，并通过认证确保其性能。V&V过程确保自主运输系统能够正确实施，而验证则侧重于评估训练有素的模型在露天矿山复杂动态条件下高效运行的能力。
